...
JNIEXPORT int Java_com_me_audioprocesser_record_AudioRecorder_startRecord(JNIEnv *env, jclass class, jstring path) {
...
JNIEXPORT int Java_com_me_audioprocesser_record_AudioRecorder_writeFrame(JNIEnv *env, jclass class, jobject frame, jint len) {
    jbyte *frameBytes = (*env)->GetDirectBufferAddress(env, frame);
    return writeFrame(frameBytes, len);
}

JNIEXPORT void Java_com_me_audioprocesser_record_AudioRecorder_stopRecord(JNIEnv *env, jclass class) {
    cleanupRecorder();
}
...
JNIEXPORT void Java_com_me_audioprocesser_play_AudioPlayer_readOpusFile(JNIEnv *env, jclass class, jobject buffer, jint capacity, jintArray args) {
...
JNIEXPORT int Java_com_me_audioprocesser_play_AudioPlayer_openOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
...
JNIEXPORT int Java_com_me_audioprocesser_play_AudioPlayer_isOpusFile(JNIEnv *env, jclass class, jstring path) {
    const char *pathStr = (*env)->GetStringUTFChars(env, path, 0);
